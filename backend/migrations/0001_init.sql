-- Таблица врачей (они же Users в будущем, но пока отдельно)
CREATE TABLE doctors (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name TEXT NOT NULL,
    specialization TEXT NOT NULL,
    bio TEXT,
    price INTEGER NOT NULL, -- цена в центах/копейках, чтобы не терять точность
    image_url TEXT,
...
CREATE TABLE appointments (
    ...
    -- Это и есть защита от коллизий на уровне БД.
    CONSTRAINT unique_doctor_time UNIQUE (doctor_id, start_time)
);

-- Добавим пару тестовых врачей сразу, чтобы не было пусто
